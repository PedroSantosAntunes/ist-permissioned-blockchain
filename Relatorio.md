...
- Adição de pending wallets para manter informação das operações pendentes para cada wallet, mesmo que esta ainda não exista. Esta estrutura existe para prevenir o edge case descrito na Nota 2.
...
- 2) Todas as transações não otimizáveis criam/atualizam as respetivas pending wallets. Isto acontece porque mesmo que uma wallet ainda não exista quando vai atualizar a informação pending, a wallet pode ser criada após esse pedido ser enviado para o sequencer mas antes de ele retornar, significando que vai existir um período em que a wallet existe mas a informação pending não estaria guardada.
